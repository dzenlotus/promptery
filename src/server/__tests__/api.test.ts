...
describe("task search endpoints", () => {
  async function setupBoardWithTasks(boardName: string, titles: string[]) {
    const board = (await (await api("POST", "/api/boards", { name: boardName })).json()) as {
      id: string;
    };
    const cols = (await (await api("GET", `/api/boards/${board.id}/columns`)).json()) as {
      id: string;
    ...
    return { boardId: board.id, columnId: cols[0]!.id, taskIds: ids };
  }

  it("GET /api/tasks/search?query= matches by title across boards", async () => {
    const a = await setupBoardWithTasks("Search-A", ["unique-needle alpha"]);
    await setupBoardWithTasks("Search-B", ["nothing here"]);

    const res = await api("GET", `/api/tasks/search?query=unique-needle`);
    expect(res.status).toBe(200);
    ...
    const ids = body.map((h) => h.task.id);
    expect(ids).toContain(a.taskIds[0]);
    expect(body.find((h) => h.task.id === a.taskIds[0])?.board.name).toBe("Search-A");
  });

  it("GET /api/tasks/search with no query lists tasks (location attached)", async () => {
    const res = await api("GET", `/api/tasks/search?limit=500`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as Array<{
      task: unknown;
    ...
    expect(body[0]).toHaveProperty("board");
  });

  it("GET /api/tasks/search filters by board_id", async () => {
    const a = await setupBoardWithTasks("Filter-A", ["alpha-tag-x"]);
    await setupBoardWithTasks("Filter-B", ["alpha-tag-x"]);

    const res = await api(
      "GET",
      `/api/tasks/search?query=alpha-tag-x&board_id=${a.boardId}`
    );
    const body = (await res.json()) as Array<{ task: { id: string } }>;
    expect(body.map((h) => h.task.id)).toEqual([a.taskIds[0]]);
  });

  it("GET /api/tasks/:id/with-location returns task + column + board", async () => {
    const { taskIds, boardId, columnId } = await setupBoardWithTasks("Loc", ["with-loc test"]);
    const res = await api("GET", `/api/tasks/${taskIds[0]}/with-location`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      task: { id: string };
      column: { id: string };
    ...
    expect(body.task.id).toBe(taskIds[0]);
    expect(body.column.id).toBe(columnId);
    expect(body.board.id).toBe(boardId);
  });

  it("/search precedence: literal route wins over /:id", async () => {
    // Empty query is allowed and should produce a 200, not "task not found"
    // (which would mean the request was routed to /:id).
